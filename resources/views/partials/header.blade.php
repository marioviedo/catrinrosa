    ...
    <div class="w-full container px-2 mx-auto fixed right-0 left-0 opacity-75">
        <div x-data="{ open: false }" class="flex flex-col bg-black text-white max-w-screen-2xl px-4 mx-auto md:items-center md:justify-between md:flex-row md:px-6 lg:px-8">
            ...
            <nav :class="{'flex': open, 'hidden': !open}" class="flex-col flex-grow divide-x gap-4 divide-pink-600 pb-4 md:pb-0 hidden md:flex md:justify-end md:flex-row">
                ...
                <div @click.away="open = false" class="relative" x-data="{ open: false }">
                  ...
                  <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48">
                    <div class="px-2 py-2 shadow bg-black">
                      <a class="block px-4 py-2 mt-2 text-lg font-semibold bg-transparent hover:text-pink-600 md:mt-0 focus:outline-none focus:shadow-outline" href="{{route('recetas.index', 'Postres')}}">Postres</a>
                      <a class="block px-4 py-2 mt-2 text-lg font-semibold bg-transparent hover:text-pink-600 md:mt-0 focus:outline-none focus:shadow-outline" href="{{route('recetas.index', 'Cocteles')}}">Cócteles</a>                    
                    </div>
                  </div>
                </div> 
                ...
                <a class="{{request()->routeIs('contacto.*') ? 'text-pink-600 ':''}}px-4 py-2 mt-2 text-lg font-semibold mr-4 hover:text-pink-600 md:mt-0 focus:outline-none focus:shadow-outline" href="{{route('contacto.index')}}">Contacto</a>
